...
export default function SupportTickets() {
  ...
  const initialTickets = [
    ...
    },
    {
      ...
      thread: [
        { who: "owner", name: "Amit Joshi", time: "2025-09-28 16:05", text: "Unable to login." },
  ...
  const lineDataSample = [
    { day: "Mon", tickets: 5 }, { day: "Tue", tickets: 7 },
    { day: "Wed", tickets: 4 }, { day: "Thu", tickets: 9 }, { day: "Fri", tickets: 6 },
  ];
  const pieData = (() => {
    const map = {};
  ...
  // ---------- ticket operations (mock: local state) ----------
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.topTabs}>
        <button className={`${styles.topTabBtn} ${activeTopTab === "dashboard" ? styles.topTabActive : ""}`} onClick={() => setActiveTopTab("dashboard")}>Dashboard</button>
        <button className={`${styles.topTabBtn} ${activeTopTab === "userQueries" ? styles.topTabActive : ""}`} onClick={() => setActiveTopTab("userQueries")}>User Queries</button>
        <button className={`${styles.topTabBtn} ${activeTopTab === "internalQueries" ? styles.topTabActive : ""}`} onClick={() => setActiveTopTab("internalQueries")}>Internal Queries</button>
      </div>
        ...
        {activeTopTab === "internalQueries" && (
          <div className={styles.tableWrap}>
            ...
            <div className={styles.internalNoteWrap}>
              ...
              <select
                value={selectedInternalTicketForNote}
      ...
      {/* ---------- DRAWER (conversation/notes/attachments) ---------- */}
      <div className={`${styles.drawer} ${drawerOpen ? styles.open : ""}`}>
        <div className={styles.drawerHeader}>
          <div>
            <div className={styles.ticketTopRow}>
              <div className={styles.ticketIdLarge}>{selectedTicket?.id}</div>
              <div><span className={`${styles.status} ${styles["status-" + (selectedTicket?.status || "").replace(/\s/g, "").toLowerCase()]}`}>{selectedTicket?.status}</span></div>
            </div>
