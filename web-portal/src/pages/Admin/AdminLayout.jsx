...
function Icon({ path, size = 18, className }) {
    return (
        <svg
            ...
            aria-hidden="true"
        >
            <path stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" d={path} />
        </svg>
    );
}
...
const NAV = {
    primary: [
        { label: "Overview", to: "/admin", icon: "M3 6h18M3 12h18M3 18h18" },
    ],
    platform: [
                    ...
                    <ul>
                        {NAV.primary.map((item) => (
                            <li key={item.label} title={item.label}>
                                <NavLink to={item.to} end className={({ isActive }) => isActive ? `${styles.navItem} ${styles.active}` : styles.navItem}>
                                    <Icon path={item.icon} />
