...
**Files inspected (read-only):**
- `AGENTS.md` - graphify requirements.
- `graphify-out/GRAPH_REPORT.md` - repo graph context.
- `apps/mobile/app.json` - Expo config and permissions.
- `apps/mobile/app/_layout.tsx` and route files - mobile navigation shape.
- `apps/mobile/services/discoveryApi.ts`, `apps/mobile/services/mockDiscovery.ts`, `apps/mobile/types/discovery.ts` - discovery contracts.
- `functions/index.js`, `functions/package.json` - callable style, region, auth, lint setup.
- `docs/SESSION_STATE.md`, `docs/TODO_NEXT_AGENT.md`, `docs/AGENT_LOG.md`, `docs/DECISIONS.md` - handoff state.

**Files created / edited:**
- `apps/mobile/package.json`, `apps/mobile/package-lock.json` - added `react-native-maps` and `expo-location`.
- `apps/mobile/app.json` - added foreground-only location permission copy for Android/iOS via Expo config.
- `apps/mobile/app/(tabs)/_layout.tsx` - added Home/Search/Stores-Map/Profile bottom tabs.
- `apps/mobile/app/(tabs)/home.tsx`, `search.tsx`, `profile.tsx`, `stores/index.tsx` - moved tab screens into the tab group and polished map/search/profile surfaces.
- `apps/mobile/app/navigation/[storeId].tsx` - upgraded to real location, route preview callable, route polyline, and Start/End in-app preview mode.
- `apps/mobile/app/results.tsx` - added demo filters for availability, open stores, verified stores, and distance/freshness sort.
- `apps/mobile/app/store/[storeId].tsx`, `apps/mobile/app/medicine/[medicineId]/stores.tsx` - added real map previews and kept Route inside Medifind.
- `apps/mobile/app/privacy.tsx` - corrected sign-out copy to Profile.
- `apps/mobile/components/RealMapView.tsx` - new reusable `react-native-maps` map with markers, polyline, and fallback.
- `apps/mobile/services/location.ts` - new foreground location helpers.
- `apps/mobile/services/routePreview.ts` - new route callable client and fallback route helper.
- `functions/index.js` - added authenticated `getRoutePreview` callable proxy for Google Routes API and declared the `GOOGLE_MAPS_ROUTES_API_KEY` Functions secret.
- `scripts/medicines/fetch_openfda_medicines.py` - new public openFDA demo seed fetcher.
- `scripts/medicines/import_medicines_to_firestore.cjs` - new dry-run-first Firestore importer for `medicines` only.
- `scripts/medicines/README.md` - seed pipeline usage and data caveats.
- `scripts/medicines/out/medicines.openfda.sample.json` - 585 unique normalized records generated by the fetch script.
- `docs/SESSION_STATE.md`, `docs/TODO_NEXT_AGENT.md`, `docs/AGENT_LOG.md` - updated handoff.

**Files intentionally NOT touched:**
- `src/**`, `public/**`, `dataconnect/**` - web portal/admin/store portal out of scope.
- `.env*`, `apps/mobile/.env`, `serviceAccountKey.json` - protected secrets/config.
- `.claude/settings.local.json`, `.codex/*.png` - protected local files.
- `firestore.rules`, `firestore.indexes.json` - no rule/index change needed.

**Decisions made:** No new decision entry. Existing D-011 supports keeping maps/routes proxies in the same `functions/` codebase.

**Verification:**
- Passed: `cd apps/mobile && npm run typecheck`.
- Passed: `cd functions && node --check index.js`.
- Passed: `cd functions && npm run lint`.
- Passed: `node --check scripts/medicines/import_medicines_to_firestore.cjs`.
- Passed after network approval: `python scripts/medicines/fetch_openfda_medicines.py --limit 1000`.
- Passed: `node scripts/medicines/import_medicines_to_firestore.cjs --project nearnest-platform --file scripts/medicines/out/medicines.openfda.sample.json --limit 100 --dry-run`.
- Passed and applied: `node scripts/medicines/import_medicines_to_firestore.cjs --project nearnest-platform --file scripts/medicines/out/medicines.openfda.sample.json --limit 100 --apply` wrote 100 `medicines` docs.
- Passed: `cd apps/mobile && npx expo export --platform android --output-dir .expo/mobile-final-map-location-export`.
- Passed: `git diff --check` with Windows CRLF/protected-file warnings only.
- Passed on retry: `graphify update .` via user-site `graphify.exe`.
- Failed/deferred: `firebase functions:secrets:set GOOGLE_MAPS_ROUTES_API_KEY --project nearnest-platform` received an empty payload from the non-interactive shell; the secret must be set manually/securely before targeted function deploy.

**Warnings for next agent:**
- A new EAS Android development build is required because native dependencies changed: `cd apps/mobile && eas build --profile development --platform android`.
- Deploy only the new route callable after setting the secret: `firebase deploy --only functions:getRoutePreview --project nearnest-platform`.
- The Routes callable needs server-side `GOOGLE_MAPS_ROUTES_API_KEY`; if missing, mobile falls back to an in-app straight route preview.

**Suggested commit message:**
`feat(mobile): add real maps location tabs and medicine seed pipeline`
